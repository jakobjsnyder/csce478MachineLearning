...
namespace hw3nb
{
    class NiaveBayes
    {
        static void Main(string[] args)
        {
            ...
            double confidence = 1.28;
            double errorDLower = errorV - confidence * Math.Sqrt((errorV * (1.0 - errorV))/testCases);
            double errorDUpper = errorV + confidence * Math.Sqrt((errorV * (1.0 - errorV)) / testCases);
            Console.WriteLine("m = " + m);
            Console.WriteLine("Num errors = " + numErrors);
            Console.WriteLine("With 80% confidence errorD(h) = [" + errorDLower + ", " + errorDUpper + "]");
            Console.ReadKey();
        }
    }
}
